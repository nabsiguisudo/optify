  ...
  return (
    <Card className="bg-white">
      <div className="mb-5 flex items-center justify-between">
        <div>
          <p className="text-lg font-semibold">{t.experimentTable.title}</p>
          <p className="text-sm text-muted-foreground">{t.experimentTable.body}</p>
        </div>
        <Badge>{t.common.detailedReportsEnabled}</Badge>
      </div>
          ...
          return (
            <div key={experiment.id} className="grid gap-4 rounded-3xl border border-border p-4 lg:grid-cols-[1.4fr_0.8fr_0.8fr_0.7fr]">
              ...
              <div>
                ...
                <Badge className={experiment.status === "running" ? "mt-2 bg-primary text-white" : "mt-2"}>{localizeStatus(experiment.status, currentLocale)}</Badge>
              </div>
              <div>
                <p className="text-xs uppercase tracking-[0.16em] text-muted-foreground">{t.common.winner}</p>
                <p className="mt-2 text-sm font-medium">{stats?.winner ? `${t.common.variant} ${stats.winner}` : t.common.noWinnerYet}</p>
                <p className="text-sm text-muted-foreground">
                  {stats?.variants[0] ? formatPercent(Math.max(...stats.variants.map((variant) => variant.conversionRate))) : "0.0%"} {t.common.peakCr}
                </p>
              </div>
